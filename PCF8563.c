...
// 2-7 and loads the data into a buffer (read_buf)

int *get_regs(uint8_t *read_buf, _tm *tm, _i2c_t *i2c){
    ...
    struct i2c_msg msgs[2] =
        {
            ...
            { .addr = PCF8583_ADDRESS, .flags = 0, .len = 2, .buf = msg_addr },
            /* Read 8-bit data */
            { .addr = PCF8583_ADDRESS, .flags = I2C_M_RD, .len = 8, .buf = read_buf},
        };
...
int get_tm(_tm *tm, _i2c_t *i2c){
  ...
  get_regs(read_buf,tm,i2c);
  
  tm->tm_sec = bcd2bin(read_buf[0] & SEC_MASK);
  tm->tm_min = bcd2bin(read_buf[1] & MIN_MASK);
  tm->tm_hour = bcd2bin(read_buf[2] & HOUR_MASK);
  tm->tm_mday = bcd2bin(read_buf[3] & DAY_MASK);
  tm->tm_wday = bcd2bin(read_buf[4] & WDAY_MASK);
  tm->tm_mon = bcd2bin(read_buf[5] & MON_MASK);
  tm->tm_year = bcd2bin(read_buf[6] & YEAR_MASK) + 2000;

  return 0;
}

//  this function sets the time, date and day of week
